...
int main(int argc, char const *argv[]) {
    ...
    if (mode == "1" || mode == "all") test_normal_echo(ip, port);
    if (mode == "2" || mode == "all") test_slow_client(ip, port);
    if (mode == "3" || mode == "all") test_idle_connection(ip, port);
    if (mode == "4" || mode == "all") test_abrupt_close(ip, port);
    if (mode == "5" || mode == "all") test_connection_storm(ip, port, 100); // 默认100个并发连接
