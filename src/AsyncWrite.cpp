...
void AsyncWriteAwaitable::await_suspend(
  ...
  if (regBuf_ == nullptr &&
      conn_->getBackpressureConfig().strategy == BackpressureStrategy::kBlock &&
    ...
    ctx.handler = [this, handle](int res) {
      ...
      if (res <= 0 || conn_->getOutputBuffer().readableBytes() <= conn_->getBackpressureConfig().outputBufferLowWaterMark) {
        // 满足条件，准备唤醒协程
