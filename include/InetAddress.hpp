...
public:
    explicit InetAddress(uint16_t port = 0, const std::string &ip = "127.0.0.1"); // 默认构造函数，ip默认是本机地址，端口号默认是0
    explicit InetAddress(const sockaddr_in &addr) : addr_(addr) {}                // 通过已有的sockaddr_in结构体构造SockAddress对象
